...
(function () {
  ...
  async function startCheckout(evt) {
    ...
    var handler = window.PaystackPop.setup({
      ...
      callback: function (response) {
        var paymentReference = response.reference || reference;
        verifyDonation(paymentReference)
          .then(function () {
            setLoading(false);
            showSuccess(state.amount, paymentReference);
          })
          .catch(function () {
            setLoading(false);
            setError('We could not verify the payment yet. Please keep your reference (' + paymentReference + ') and contact support if this persists.');
          });
      },
      onClose: function () {
        setLoading(false);
  ...
  // The browser callback is not authoritative. Confirm the charge with our
  // server, which verifies it directly with Paystack before showing success.
  async function verifyDonation(reference) {
    var response = await fetch('/api/paystack-donation-verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ reference: reference })
    });
    var result = await response.json().catch(function () { return {}; });
    if (!response.ok || !result.success) throw new Error(result.message || 'Verification failed');
  }
